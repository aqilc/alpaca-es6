...
export class Client extends EventEmitter {
  ...
  constructor({ key, secret, paper = true, rate_limit = true, cache = false, auto = false }) {
    ...
    // Sets up streams
    this.ws.account = new Account(this);
    this.ws.market = new Market(this);
  ...
  connect() {
    ...
    this.ws.account.connect().on("trade_updates", m => this.emit("trade_updates", m)).on("account_updates", m => this.emit("account_updates", m));
    this.ws.market.connect().on("trade", m => this.emit("trades", m)).on("quote", m => this.emit("quote", m)).on("minute", m => this.emit("minute", m));
